// Integer modulo by a constant power of two (LModPowTwoI) with a negative
// dividend: the dividend's sign test must be a 32-bit test. A full-width
// 64-bit sign test misclassifies an int32 held zero-extended in its register
// and returns the masked positive value `x & (2^n - 1)` instead of the
// negative remainder (seen on PPC64, fixed with branch32). The reference uses
// a non-constant divisor, which takes the divide-based path (LModI).
