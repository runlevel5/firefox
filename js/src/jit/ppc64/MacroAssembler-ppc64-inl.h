...
void MacroAssembler::add32(Imm32 imm, Register src, Register dest) {
  // Plain value transfer only: add32 canonicalizes with its trailing extsw,
  // so move32's extsw here would be redundant (and a no-op-with-cost when
  // src == dest, the common increment shape).
  if (src != dest) {
    as_or_(dest, src, src);
  }
  add32(imm, dest);
...
void MacroAssembler::lshift64(Imm32 imm, Register64 src, Register64 dest) {
  MOZ_ASSERT(0 <= imm.value && imm.value < 64);
  x_sldi(dest.reg, src.reg, imm.value);
}
...
void MacroAssembler::rshift64(Imm32 imm, Register64 src, Register64 dest) {
  MOZ_ASSERT(0 <= imm.value && imm.value < 64);
  x_srdi(dest.reg, src.reg, imm.value);
}
...
void MacroAssembler::rshift64Arithmetic(Imm32 imm, Register64 src,
                                        Register64 dest) {
  MOZ_ASSERT(0 <= imm.value && imm.value < 64);
  as_sradi(dest.reg, src.reg, imm.value);
}
...
void MacroAssembler::ctz64(Register64 src, Register64 dest) {
  if (HasPOWER9()) {
    ...
    UseScratchRegisterScope temps(*this);
    Register tmp = temps.Acquire();
    // popcnt((x-1) & ~x) counts the trailing zeros, including x==0 -> 64
...
void MacroAssembler::ctz32(Register src, Register dest, bool knownNotZero) {
  if (HasPOWER9()) {
    ...
    UseScratchRegisterScope temps(*this);
    Register tmp = temps.Acquire();
    // popcnt((x-1) & ~x) of the low word counts the trailing zeros,
    // including x==0 -> 32. The 64-bit compute leaves an arbitrary count
...
void MacroAssembler::branch32(Condition cond, const AbsoluteAddress& lhs,
  ...
  UseScratchRegisterScope temps(asMasm());
  Register scratch = temps.Acquire();
  ma_loadAbsAddr(scratch, lhs.addr);
...
void MacroAssembler::branchAdd32(Condition cond, T src, Register dest,
  ...
  switch (cond) {
    case Overflow: {
      // Do a raw 64-bit add (no sign extension) so we can detect 32-bit
      // overflow. Both inputs should already be sign-extended 32-bit values,
      // so the 64-bit sum in the scratch is mathematically correct, and its
      // extsw in dest is the canonical wrapped result on both branch
      // outcomes. The two differ exactly when the 32-bit add overflowed.
      UseScratchRegisterScope temps(asMasm());
      Register scratch = temps.Acquire();
      MOZ_ASSERT(scratch != dest);
      if constexpr (std::is_same_v<T, Imm32>) {
        if (is_intN(int64_t(src.value), 16)) {
          as_addi(scratch, dest, int16_t(src.value));
        } else {
          xs_mr(scratch, dest);
          addPtr(src, scratch);
        }
      } else {
        as_add(scratch, dest, src);
      }
      as_extsw(dest, scratch);
      as_cmpd(scratch, dest);
...
void MacroAssembler::branchTest32(Condition cond, const Address& lhs, Imm32 rhs,
  ...
  Register scratch = temps.Acquire();
  load32(lhs, scratch);
  // Zero/NonZero with a 16-bit mask: andi. sets CR0 directly, saving the
  // extsw + cmpdi that the generic and32 path needs.
  if ((cond == Zero || cond == NonZero) && is_uintN(rhs.value, 16)) {
    as_andi_rc(scratch, scratch, uint16_t(rhs.value));
    ma_b(cond == Zero ? Equal : NotEqual, label);
    return;
  }
...
void MacroAssembler::branchTest32(Condition cond, const AbsoluteAddress& lhs,
  ...
  Register scratch = temps.Acquire();
  ma_loadAbsAddr(scratch, lhs.addr);
  branchTest32(cond, Address(scratch, 0), rhs, label);
}
...
FaultingCodeRange MacroAssembler::storeFloat32(FloatRegister src,
  ...
  if (is_intN(addr.offset, 16)) {
    return FaultingCodeRange(as_stfs(src, addr.base, addr.offset).getOffset());
  }
  if (HasPOWER10() && is_intN((intptr_t)addr.offset, 34)) {
    return FaultingCodeRange(
  ...
  return FaultingCodeRange(as_stfsx(src, addr.base, scratch).getOffset());
}
...
FaultingCodeRange MacroAssembler::storeFloat32(FloatRegister src,
  ...
  return FaultingCodeRange(as_stfs(src, scratch, 0).getOffset());
}
...
FaultingCodeRange MacroAssembler::storeDouble(FloatRegister src,
  ...
  MOZ_ASSERT(addr.base != r0);
  if (is_intN(addr.offset, 16)) {
    return FaultingCodeRange(as_stfd(src, addr.base, addr.offset).getOffset());
  }
  if (HasPOWER10() && is_intN((intptr_t)addr.offset, 34)) {
    return FaultingCodeRange(
  ...
  Register scratch = temps.Acquire();
  movePtr(ImmWord(addr.offset), scratch);
  return FaultingCodeRange(as_stfdx(src, addr.base, scratch).getOffset());
}
...
FaultingCodeRange MacroAssembler::storeDouble(FloatRegister src,
  ...
  Register scratch = temps.Acquire();
  computeEffectiveAddress(addr, scratch);
  return FaultingCodeRange(as_stfd(src, scratch, 0).getOffset());
}
...
FaultingCodeRange MacroAssembler::storeFloat16(FloatRegister src,
  ...
  MOZ_ASSERT(HasPOWER9());
  if (dest.offset == 0) {
    return FaultingCodeRange(as_stxsihx(src, r0, dest.base).getOffset());
  }
  if (is_intN(dest.offset, 16)) {
    as_addi(temp, dest.base, dest.offset);
    return FaultingCodeRange(as_stxsihx(src, r0, temp).getOffset());
  }
  movePtr(ImmWord(dest.offset), temp);
  return FaultingCodeRange(as_stxsihx(src, dest.base, temp).getOffset());
}
...
FaultingCodeRange MacroAssembler::loadUnalignedSimd128(const Address& src,
  ...
  if (HasPOWER9()) {
    ...
    if (src.offset == 0) {
      ...
      return FaultingCodeRange(as_lxvx(dest, r0, src.base).getOffset());
    }
    if ((src.offset & 15) == 0 && src.offset >= -32768 &&
        src.offset <= 32752 && src.base != r0) {
      // DQ-form takes the 16-aligned displacement directly, sparing the
      // scratch GPR the X-form needs (register spill slots qualify).
      return FaultingCodeRange(as_lxv(dest, src.base, src.offset).getOffset());
    }
    Register scratch = temps.Acquire();
    movePtr(ImmWord(src.offset), scratch);
    return FaultingCodeRange(as_lxvx(dest, src.base, scratch).getOffset());
  }
  ...
  Register scratch = temps.Acquire();
  FaultingCodeRange fco;
  if (src.offset == 0) {
    fco = FaultingCodeRange(as_lxvd2x(dest, r0, src.base).getOffset());
  } else {
    movePtr(ImmWord(src.offset), scratch);
    fco = FaultingCodeRange(as_lxvd2x(dest, src.base, scratch).getOffset());
  }
...
FaultingCodeRange MacroAssembler::loadUnalignedSimd128(const BaseIndex& src,
  ...
  if (HasPOWER9()) {
    return FaultingCodeRange(as_lxvx(dest, r0, scratch).getOffset());
  }
  FaultingCodeRange fco(as_lxvd2x(dest, r0, scratch).getOffset());
#if !(defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__)
  as_xxpermdi(dest, dest, dest, 2);
#endif
  return fco;
...
FaultingCodeRange MacroAssembler::storeUnalignedSimd128(FloatRegister src,
  ...
  if (HasPOWER9()) {
    if (dest.offset == 0) {
      return FaultingCodeRange(as_stxvx(src, r0, dest.base).getOffset());
    }
    if ((dest.offset & 15) == 0 && dest.offset >= -32768 &&
        dest.offset <= 32752 && dest.base != r0) {
      return FaultingCodeRange(as_stxv(src, dest.base, dest.offset).getOffset());
    }
    Register scratch = temps.Acquire();
    movePtr(ImmWord(dest.offset), scratch);
    return FaultingCodeRange(as_stxvx(src, dest.base, scratch).getOffset());
  }
  ...
  FaultingCodeRange fco;
  if (dest.offset == 0) {
    fco = FaultingCodeRange(as_stxvd2x(toStore, r0, dest.base).getOffset());
  } else {
    movePtr(ImmWord(dest.offset), scratch);
    fco = FaultingCodeRange(
        as_stxvd2x(toStore, dest.base, scratch).getOffset());
  }
  return fco;
}

FaultingCodeRange MacroAssembler::storeUnalignedSimd128(
  ...
  if (HasPOWER9()) {
    return FaultingCodeRange(as_stxvx(src, r0, scratch).getOffset());
  }
#if defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  // stxvd2x is an identity 16-byte store on big-endian.
  return FaultingCodeRange(as_stxvd2x(src, r0, scratch).getOffset());
#else
  ScratchSimd128Scope scratch128(*this);
  as_xxpermdi(scratch128, src, src, 2);
  return FaultingCodeRange(as_stxvd2x(scratch128, r0, scratch).getOffset());
#endif
}
...
// Splat a 32-bit immediate into all four word elements without touching
// memory or the constant pool. Endian-independent: the architectural
// word-splat image equals the canonical (wasm-lane) uniform image on both
// endians. Clobbers SecondScratchReg on pre-POWER10. mtvsrws (ISA 3.0)
// splats GPR bits 32:63; POWER8 places the value in word element 1 with
// mtvsrd and splats it with xxspltw.
static void SplatWordImm(MacroAssembler& masm, uint32_t word,
                         FloatRegister dest) {
  if (HasPOWER10()) {
    masm.as_xxspltiw(dest, word);
    return;
  }
  masm.move32(Imm32(word), SecondScratchReg);
  if (HasPOWER9()) {
...
void MacroAssembler::loadConstantSimd128(const SimdConstant& v,
                                         FloatRegister dest) {
  // Splat-encodable constants synthesize inline with no pool entry and no
  // memory access. The canonical register image is identical on both
  // endians (pool bytes are pre-reversed on BE), so value-level pattern
  // detection is endian-independent; the wasm-lane to architectural-word
  // mapping is word j = lane 3-j (see the lane-mapping comment below).
  const uint8_t* b = static_cast<const uint8_t*>(v.bytes());
  uint32_t lane0 = mozilla::LittleEndian::readUint32(b);
  uint32_t lane1 = mozilla::LittleEndian::readUint32(b + 4);
  if (memcmp(b, b + 4, 4) == 0 && memcmp(b, b + 8, 4) == 0 &&
      memcmp(b, b + 12, 4) == 0) {
    SplatWordImm(*this, lane0, dest);
    return;
  }
  if (HasPOWER10() && memcmp(b, b + 8, 4) == 0 &&
      memcmp(b + 4, b + 12, 4) == 0) {
    as_xxsplti32dx(dest, /*ix=*/false, lane1);
    as_xxsplti32dx(dest, /*ix=*/true, lane0);
    return;
  }
...
// signed immediate to all lanes in 1 insn with no pool entry. Wider values
// go through SplatWordImm (uniform bytes/halfwords are a uniform word).
static void SplatImm8(MacroAssembler& masm, Imm32 imm, FloatRegister dest) {
  ...
  SplatWordImm(masm, uint8_t(val) * 0x01010101u, dest);
}
...
static void SplatImm16(MacroAssembler& masm, Imm32 imm, FloatRegister dest) {
  ...
  uint32_t v = uint16_t(val);
  SplatWordImm(masm, (v << 16) | v, dest);
}
...
static void SplatImm32(MacroAssembler& masm, Imm32 imm, FloatRegister dest) {
  ...
  // The architectural word-splat layout is also what the vsl*/vsr*
  // shift-count consumers require on BE, where the byte-reversing
  // constant-pool path would misplace the count (the per-doubleword count
  // for i64 shifts then reads as 0).
  SplatWordImm(masm, uint32_t(val), dest);
}
...
void MacroAssembler::replaceLaneInt32x4(unsigned lane, Register rhs,
  ...
  // Modeled on replaceLaneInt16x8 above.
  UseScratchRegisterScope temps(asMasm());
  ScratchSimd128Scope scratch128(*this);
  Register tmp = temps.Acquire();
...
//
// Only ScratchSimd128Reg and the red zone may be used as scratch here: the
// allocatable VRs may hold a live wasm v128. Stash src to red-zone slot 0 so
// dest can be freely overwritten; vmul-even (signed/unsigned) of src with
// splat(1) — `vspltis{b,h}` — produces the sign/zero-extended even-lane
// products into dest; stash that to slot 1 and rebuild scratch=src (slot 0)
// and dest=splat(1); vmul-odd produces the odd products; restore even from
// slot 1 and pairwise-add.
