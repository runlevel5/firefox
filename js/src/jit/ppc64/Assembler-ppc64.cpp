...
BufferOffset Assembler::as_andc(Register rd, Register rs, Register rb) {
  spew("andc\t%3s,%3s,%3s", rd.name(), rs.name(), rb.name());
  return writeInst(InstReg(PPC_andc, rs, rd, rb).encode());
}
...
// stdu is DS-form: the displacement is a 14-bit field and its low two bits
// must be zero, so it needs the alignment guard the generic macro omits.
BufferOffset Assembler::as_stdu(Register rd, Register rb, int16_t off) {
  spew("stdu\t%3s,%d(%3s)", rd.name(), off, rb.name());
  MOZ_ASSERT(rb != r0);
  MOZ_ASSERT(!(off & 0x03));
  return writeInst(InstImm(PPC_stdu, rd, rb, off).encode());
}
...
BufferOffset Assembler::as_xsmaxdp(FloatRegister xt, FloatRegister xa,
                                   FloatRegister xb) {
  spew("xsmaxdp\t%3s,%3s,%3s", xt.name(), xa.name(), xb.name());
  return writeInst(XX3Form(PPC_xsmaxdp, xt, xa, xb));
}

BufferOffset Assembler::as_xsmindp(FloatRegister xt, FloatRegister xa,
                                   FloatRegister xb) {
  spew("xsmindp\t%3s,%3s,%3s", xt.name(), xa.name(), xb.name());
  return writeInst(XX3Form(PPC_xsmindp, xt, xa, xb));
}
...
// DQ-form: (61 << 26) | T << 21 | RA << 16 | DQ << 4 | TX << 3 | XO,
// XO = 1 (lxv) / 5 (stxv). DQ = offset / 16, so (offset & 0xFFF0) is the
// already-placed field.
BufferOffset Assembler::as_lxv(FloatRegister xt, Register ra, int32_t offset) {
  MOZ_ASSERT((offset & 15) == 0);
  MOZ_ASSERT(offset >= -32768 && offset <= 32752);
  MOZ_ASSERT(ra != r0);
  spew("lxv\t%3s,%d(%s)", xt.name(), offset, ra.name());
  uint32_t enc = uint32_t(xt.encoding());
  return writeInst((61u << 26) | ((enc & 31) << 21) |
                   (uint32_t(ra.code()) << 16) | (uint32_t(offset) & 0xFFF0) |
                   (((enc >> 5) & 1) << 3) | 1u);
}

BufferOffset Assembler::as_stxv(FloatRegister xs, Register ra, int32_t offset) {
  MOZ_ASSERT((offset & 15) == 0);
  MOZ_ASSERT(offset >= -32768 && offset <= 32752);
  MOZ_ASSERT(ra != r0);
  spew("stxv\t%3s,%d(%s)", xs.name(), offset, ra.name());
  uint32_t enc = uint32_t(xs.encoding());
  return writeInst((61u << 26) | ((enc & 31) << 21) |
                   (uint32_t(ra.code()) << 16) | (uint32_t(offset) & 0xFFF0) |
                   (((enc >> 5) & 1) << 3) | 5u);
}
...
BufferOffset Assembler::emitPrefixedInst(uint32_t prefix, uint32_t suffix) {
...
BufferOffset Assembler::as_paddi(Register rt, Register ra, int64_t imm34,
  ...
  return emitPrefixedInst(prefix, suffix);
}
...
// xxspltiw XT, IMM32   (8RR:D, prefix type 1; splat IMM32 into all four
// word elements)
BufferOffset Assembler::as_xxspltiw(FloatRegister xt, uint32_t imm) {
  spew("xxspltiw\t%3s,0x%08x", xt.name(), imm);
  uint32_t enc = uint32_t(xt.encoding());
  uint32_t prefix = (1u << 26) | (1u << 24) | (imm >> 16);
  uint32_t suffix = (32u << 26) | ((enc & 31) << 21) | (3u << 17) |
                    (((enc >> 5) & 1) << 16) | (imm & 0xFFFF);
  return emitPrefixedInst(prefix, suffix);
}

// xxsplti32dx XT, IX, IMM32   (8RR:D; write IMM32 into word elements IX
// and IX+2 only, preserving the other two)
BufferOffset Assembler::as_xxsplti32dx(FloatRegister xt, bool ix,
                                       uint32_t imm) {
  spew("xxsplti32dx\t%3s,%d,0x%08x", xt.name(), ix ? 1 : 0, imm);
  uint32_t enc = uint32_t(xt.encoding());
  uint32_t prefix = (1u << 26) | (1u << 24) | (imm >> 16);
  uint32_t suffix = (32u << 26) | ((enc & 31) << 21) |
                    ((ix ? 1u : 0u) << 17) | (((enc >> 5) & 1) << 16) |
                    (imm & 0xFFFF);
  return emitPrefixedInst(prefix, suffix);
}
...
static uint64_t* PoolEntryFromGPR64Load(Instruction* inst0);
...
bool Assembler::PatchConstantPoolLoad(void* loadAddr, void* constPoolAddr,
  ...
  if (loadType == PoolLoadGPR64) {
    // Same three emission shapes as the FPR case below, but loading a GPR:
    // POWER10 pld (8LS-form prefix, unlike the FP loads' MLS form), POWER9
    // addpcis + ld, POWER8 bcl + mflr + ld. ld is DS-form; the low two
    // displacement bits are zero because pool entries are 4-byte aligned
    // relative to the load, and the 8-byte entry itself is 4-byte units.
    uint32_t baseReg = SavedScratchRegister.code();

    if (HasPOWER10()) {
      bool needLeadingNop = (loadOffset & 15) == 12;
      ...
      // Type 0 (8LS), R=1, RA=0; suffix is pld (primary opcode 57).
      uint32_t prefix = (1u << 26) | (0u << 24) | (1u << 20) | (d0 & 0x3FFFFu);
      uint32_t suffix = (57u << 26) | (destReg << 21) | d1;
      ...
      MOZ_ASSERT((lo & 3) == 0);
      // Self-based like the load64 stanza: the destination doubles as the
      // address base, so no other register is touched. r16 must NOT be used
      // here: these loads appear in interpreter-entry trampolines that run
      // without an enterJit-style non-volatile save, and r16 is callee-saved
      // in the C ABI (clobbering it corrupts the C++ caller's state).
      // [0] addpcis rD, hi
      uint32_t Dhi = uint16_t(hi);
      inst[0] = (19u << 26) | (destReg << 21) | ((Dhi >> 1) & 0x1F) << 16 |
                ((Dhi >> 6) & 0x3FF) << 6 | (2u << 1) | (Dhi & 1u);
      // [1] ld rD, lo(rD) — DS-form, primary opcode 58, XO=0.
      inst[1] = (58u << 26) | (destReg << 21) | (destReg << 16) |
                (uint16_t(lo) & 0xFFFC);
    } else {
      MOZ_ASSERT(displacement >= -32768 && displacement < 32768);
      MOZ_ASSERT((displacement & 3) == 0);
      // [0] bcl 20,0,$+4
      inst[0] = PPC_bcl_always_plus4;
      // [1] mflr r16
      inst[1] = PPC_mfspr | (baseReg << 21) | PPC_SPR(spr_lr);
      // [2] ld rD, displacement(r16)
      inst[2] = (58u << 26) | (destReg << 21) | (baseReg << 16) |
                (displacement & 0xFFFC);
    }

#ifdef DEBUG
    // Round-trip self-check: the data-relocation reader must find the exact
    // pool entry this patch aimed at, on both endiannesses.
    if (HasPOWER9()) {
      uint64_t* entry = PoolEntryFromGPR64Load((Instruction*)inst);
      MOZ_ASSERT(entry == (uint64_t*)((uint8_t*)constPoolAddr + index * 4),
                 "PoolLoadGPR64 patch/decode mismatch");
    }
#endif
  } else if (loadType == PoolLoadFPR64 || loadType == PoolLoadFPR32) {
    ...
    if (HasPOWER10()) {
      ...
      // The alignment decision must use the LOGICAL buffer offset: the final
      // executable base is CodeAlignment (16)-aligned, so the logical offset
      // carries the executed mod-16 phase. loadAddr is a chunk-internal
      // pointer whose phase is unrelated (this was a real-P10 SIGBUS: the
      // prefix straddled a 64-byte fetch boundary in copied wasm code).
      bool needLeadingNop = (loadOffset & 15) == 12;
      int prefixSlot = needLeadingNop ? 1 : 0;
      int prefixByteOffset = prefixSlot * 4;
      int64_t SI = int64_t(displacement) + 4 - prefixByteOffset;
...
// Recognize a patched PoolLoadGPR64 site — pld (possibly behind the
// alignment nop) or addpcis r16 + ld rD, lo(r16) — and return the address of
// its 8-byte pool entry. The addpcis pool form is distinguished from the
// addpcis load64 stanza by the ld base register: the pool load uses r16
// (SavedScratchRegister, never an allocatable destination), the stanza loads
// through its own destination. Returns nullptr for stanza shapes.
static uint64_t* PoolEntryFromGPR64Load(Instruction* inst0) {
  uint32_t w0 = inst0->encode();
  Instruction* prefixAt = nullptr;
  if (((w0 >> 26) & 0x3f) == 1) {
    prefixAt = inst0;
  } else if (w0 == NopInst && ((inst0[1].encode() >> 26) & 0x3f) == 1) {
    prefixAt = inst0 + 1;
  }
  if (prefixAt) {
    uint32_t p = prefixAt->encode();
    uint32_t sfx = prefixAt[1].encode();
    // 8LS-form prefix (type 0), R=1, with a pld suffix (primary opcode 57).
    if (((p >> 24) & 0x3) != 0 || !((p >> 20) & 1) ||
        ((sfx >> 26) & 0x3f) != 57) {
      return nullptr;
    }
    int64_t d = (int64_t(p & 0x3FFFF) << 16) | (sfx & 0xFFFF);
    d = (d << 30) >> 30;
    return (uint64_t*)((uint8_t*)prefixAt + d);
  }
  if (((w0 >> 26) & 0x3f) == 19) {
    // addpcis rX + ld rD, d(rX) where rX is the addpcis target. This covers
    // both the pool shape (rX == rD, self-based) and the legacy load64
    // stanza (whose quad the same displacement math reaches), so no shape
    // discrimination is needed.
    uint32_t w1 = inst0[1].encode();
    if (((w1 >> 26) & 0x3f) == 58 &&
        ((w1 >> 16) & 0x1f) == ((w0 >> 21) & 0x1f) && (w1 & 3) == 0) {
      // Invert the DX-form immediate split used by PatchConstantPoolLoad:
      // D = (bits 6..15 << 6) | (bits 16..20 << 1) | bit 0.
      int32_t hi = int16_t(uint16_t((((w0 >> 6) & 0x3FF) << 6) |
                                    (((w0 >> 16) & 0x1F) << 1) | (w0 & 1)));
      int32_t lo = int16_t(uint16_t(w1 & 0xFFFC));
      return (uint64_t*)((uint8_t*)inst0 + 4 + (hi << 16) + lo);
    }
  }
  return nullptr;
}
...
uint64_t Assembler::ExtractLoad64Value(Instruction* inst0) {
  if (uint64_t* entry = PoolEntryFromGPR64Load(inst0)) {
    uint64_t v;
...
void Assembler::UpdateLoad64Value(Instruction* inst0, uint64_t value) {
  if (uint64_t* entry = PoolEntryFromGPR64Load(inst0)) {
    // The value lives in the pool as plain data read by ld/pld; no ICache
    // flush is needed for an update.
