...
void Simulator::decodeVSX(SimInstruction* instr) {
  ...
  switch (xo) {
    ...
    // === XX3-form scalar: xsmaxdp / xsmindp (ISA 2.06 base forms) ===
    //
    // An SNaN operand wins (returned quieted, first-operand precedence);
    // a QNaN operand loses to the numeric operand (minNum/maxNum style);
    // both-QNaN returns src1; ±0 ties follow signed-zero ordering
    // (max -> +0, min -> -0).
    case 256: case 257:    // xsmaxdp  (XO8=128 → 9-bit 256/257)
    case 272: case 273: {  // xsmindp  (XO8=136 → 9-bit 272/273)
      int xa = int(instr->raValue() | (instr->bit(2) << 5));
      int xb = int(instr->rbValue() | (instr->bit(1) << 5));
      int xt = int(instr->rtValue() | (instr->bit(0) << 5));
      ...
      memcpy(&b, bb + 8, 8);
      bool isMax = (xo >> 1) == 128;
      auto isSNaN = [](double v) {
        uint64_t bits;
        memcpy(&bits, &v, 8);
        return std::isnan(v) && !(bits & 0x0008000000000000ull);
      };
      auto quiet = [](double v) {
        uint64_t bits;
        memcpy(&bits, &v, 8);
...
void Simulator::decodePrefixed(SimInstruction* prefix) {
  ...
  } else if (type == 1 && suffixOp6 == 32) {
    // 8RR:D splat-immediate forms. TX at suffix bit 16; subop at suffix
    // bits 20-17 (3 = xxspltiw, 0/1 = xxsplti32dx with IX). imm32 is
    // prefix bits 15-0 || suffix bits 15-0.
    int xt = static_cast<int>(rt | (suffix->bit(16) << 5));
    uint32_t subop = suffix->bits(20, 17);
    uint32_t imm32 = ((d0 & 0xFFFFu) << 16) | d1;
    uint8_t buf[16];
    if (subop == 3) {
      for (int i = 0; i < 4; i++) {
        memcpy(buf + 4 * i, &imm32, 4);
      }
    } else if (subop <= 1) {
      // Architectural words IX and IX+2 = canonical (wasm-order) word
      // lanes 3-IX and 1-IX; the other two words are preserved.
      getVSR128(xt, buf);
      memcpy(buf + 4 * (3 - subop), &imm32, 4);
      memcpy(buf + 4 * (1 - subop), &imm32, 4);
    } else {
      MOZ_CRASH("decodePrefixed: unimplemented 8RR splat subop");
    }
    setVSR128(xt, buf);
  } else {
    MOZ_CRASH_UNSAFE_PRINTF(
...
void Simulator::instructionDecode(SimInstruction* instr) {
  ...
  switch (opcode) {
    ...
    // DQ-form VSX load/store (ISA 3.0): lxv (XO=1) / stxv (XO=5). The JIT
    // never emits the DS-form stxsd/stxssp that share this opcode.
    case 61: {
      uint32_t bits = instr->instructionBits();
      uint32_t xo3 = bits & 7;
      int vsr = int(instr->rtValue() | (((bits >> 3) & 1) << 5));
      int32_t offset = int32_t(int16_t(bits & 0xFFF0));
      uint32_t ra = instr->raValue();
      uint64_t ea = (ra == 0 ? 0 : U64(getRegister(ra))) + int64_t(offset);
      if (handleWasmSegFault(ea, 16)) break;
      uint8_t buf[16];
      if (xo3 == 1) {
        memcpy(buf, reinterpret_cast<const void*>(ea), 16);
        ...
        MOZ_CRASH_UNSAFE_PRINTF(
            "instructionDecode: unsupported opcode-61 XO %u (0x%08x)", xo3,
            bits);
      }
      break;
    }
