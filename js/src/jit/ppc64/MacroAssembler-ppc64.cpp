...
void MacroAssemblerPPC64Compat::convertDoubleToPtr(FloatRegister src,
  ...
  // fctidz saturates src == 2^63 to INT64_MAX, which fcfid rounds straight
  // back to 2^63, so the round-trip check above wrongly passes. INT64_MAX
  // is not representable as a double, so a passing round-trip can only
  // produce it through that saturation; reject it.
  asMasm().branchPtr(Assembler::Equal, dest, ImmWord(INT64_MAX), fail);
...
FaultingCodeRange MacroAssembler::wasmTrapInstruction() {
  ...
  FaultingCodeRange fco = FaultingCodeRange(currentOffset());
  xs_trap();
  return fco;
}
...
// computation (Bug 2056833). Uses no extra scratch registers.
static void ComputeWasmAtomicAddress(MacroAssembler& masm, const Address& mem,
                                     Register dest, wasm::ZeroExtendIndex) {
  masm.computeEffectiveAddress(mem, dest);
}
static void ComputeWasmAtomicAddress(MacroAssembler& masm, const BaseIndex& mem,
                                     Register dest,
                                     wasm::ZeroExtendIndex zeroExtend) {
  if (zeroExtend == wasm::ZeroExtendIndex::Yes) {
    MOZ_ASSERT(mem.scale == TimesOne);
    masm.as_rldicl(dest, mem.index, 0, 32);
    masm.as_add(dest, mem.base, dest);
    if (mem.offset) {
      masm.addPtr(Imm32(mem.offset), dest);
    }
    return;
  }
  masm.computeEffectiveAddress(mem, dest);
...
static void CompareExchange(MacroAssembler& masm,
  ...
  Register scratch = temps.Acquire();
  ComputeWasmAtomicAddress(masm, mem, scratch, zeroExtend);
...
static void AtomicExchange(MacroAssembler& masm,
  ...
  ComputeWasmAtomicAddress(masm, mem, memTemp, zeroExtend);
...
void MacroAssembler::wasmAtomicExchange64(const wasm::MemoryAccessDesc& access,
                                          ...
                                          Register64 output,
                                          wasm::ZeroExtendIndex zeroExtend) {
  AtomicExchange64(*this, &access, access.sync(), mem, src, output, zeroExtend);
}
...
// Ion-facing variants carrying the index-extension request; see
// ComputeWasmAtomicAddress.
void MacroAssembler::wasmCompareExchange(const wasm::MemoryAccessDesc& access,
                                         const BaseIndex& mem, Register oldval,
                                         Register newval, Register valueTemp,
                                         Register offsetTemp, Register maskTemp,
                                         ...
                                         wasm::ZeroExtendIndex zeroExtend) {
  CompareExchange(*this, &access, access.type(), access.sync(), mem, oldval,
                  newval, valueTemp, offsetTemp, maskTemp, output, zeroExtend);
}
...
void MacroAssembler::wasmAtomicExchange(const wasm::MemoryAccessDesc& access,
                                        ...
                                        wasm::ZeroExtendIndex zeroExtend) {
  AtomicExchange(*this, &access, access.type(), access.sync(), mem, value,
                 valueTemp, offsetTemp, maskTemp, output, zeroExtend);
}
...
void MacroAssembler::wasmAtomicEffectOp(const wasm::MemoryAccessDesc& access,
                                        ...
                                        wasm::ZeroExtendIndex zeroExtend) {
  AtomicEffectOp(*this, &access, access.type(), access.sync(), op, mem, value,
                 valueTemp, offsetTemp, maskTemp, zeroExtend);
}
