...
static constexpr Register CallReg = r12;
...
class MacroAssemblerPPC64Compat : public MacroAssemblerPPC64 {
 public:
  ...
  // and dereferences a C-function descriptor on ELFv1. See the
  ...
  // Byte-reverse the 16 bytes of |src| into |dest|, converting between the
  // raw native image of little-endian v128 memory and the canonical LE
  // register layout. xxbrq on POWER9+; on POWER8 a vperm whose control
  // vector is pool-loaded into v0, so |src| must not be v0 there (|dest|
  // may alias |src| or v0).
  void byteReverseSimd128(FloatRegister src, FloatRegister dest);
#endif
  ...
  void convertInt32ToDouble(const Address& srcArg, FloatRegister dest) {
    // On PUNBOX64 the int32 lives in the low word of an 8-byte slot, at byte
    // +4 on big-endian: either the payload of a boxed Value (ensureDouble) or
    // a raw int32 spilled through a 64-bit push (CacheIR PayloadStack). Mirror
  ...
  FaultingCodeRange loadFloat16(const Address& addr, FloatRegister dest,
    ...
    MOZ_ASSERT(HasPOWER9());
    if (addr.offset == 0) {
      return FaultingCodeRange(as_lxsihzx(dest, r0, addr.base).getOffset());
    }
    if (is_intN(addr.offset, 16)) {
      as_addi(temp, addr.base, addr.offset);
      return FaultingCodeRange(as_lxsihzx(dest, r0, temp).getOffset());
    }
    movePtr(ImmWord(addr.offset), temp);
    return FaultingCodeRange(as_lxsihzx(dest, addr.base, temp).getOffset());
  }
  ...
  void movePtr(ImmWord imm, Register dest) {
    ...
    } else if ((uint32_t)imm.value == 0) {
      // High half only (the shifted-tag shape from Value boxing): build the
      // high 32 bits in the low half, then shift up. The rotate discards any
      // sign-extension garbage.
      uint32_t hi32 = (uint32_t)(imm.value >> 32);
      if (HasPOWER10()) {
        as_paddi(dest, r0, (int64_t)hi32, /*R=*/false);
      } else {
        xs_lis(dest, (int16_t)(hi32 >> 16));
        if (hi32 & 0xFFFF) {
          as_ori(dest, dest, hi32 & 0xFFFF);
        }
      }
      as_rldicr(dest, dest, 32, 31);
    } else if (HasPOWER10()) {
      // Both 32-bit halves via pli (non-negative, so they fit the 34-bit
      // signed field), merged with rldimi. Same 20 bytes as the 5-insn
      // chain but two fewer dispatch slots. Temp choice as below.
      Register temp = (dest != SecondScratchReg) ? SecondScratchReg : r0;
      as_paddi(dest, r0, (int64_t)(uint32_t)imm.value, /*R=*/false);
      as_paddi(temp, r0, (int64_t)(uint32_t)(imm.value >> 32), /*R=*/false);
  ...
  // Load a 64-bit GPR constant (typically an absolute runtime address that a
  // hot loop reloads, like the interrupt or incremental-barrier flag) from
  // the inline constant pool. Same emission shapes as loadFromPoolFloat64:
  // pld pc-relative on POWER10, addpcis + ld on POWER9. Callers must check
  // for POWER8 and pool inhibition themselves.
  BufferOffset loadFromPoolGPR64(Register dest, uint64_t value) {
    size_t slots = HasPOWER10() ? 3 : 2;
    uint32_t hint = (uint32_t(dest.code()) << 16) |
                    (uint32_t(PoolLoadGPR64) << 21) | 0xF0000000;
    uint32_t inst[3] = {hint, NopInst, NopInst};
    return m_buffer.allocEntry(slots, 2, (uint8_t*)inst, (uint8_t*)&value);
  }
  // Materialize the address of an absolute runtime location, via the pool on
  // POWER9+ (addpcis + ld, or a single prefixed pld) rather than the 3-5
  // instruction immediate build. Not for patchable or serialized code; the
  // AbsoluteAddress branch helpers are the intended callers.
  void ma_loadAbsAddr(Register dest, const void* addr) {
    if (HasPOWER9() && !m_buffer.poolsInhibited() && !oom()) {
      loadFromPoolGPR64(dest, uint64_t(uintptr_t(addr)));
    } else {
...
#if defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
    // Store the constant byte-reversed: a raw pool load (lxvx on POWER9+,
    // identity lxvd2x on POWER8) then yields the canonical little-endian
    // register layout directly, with no runtime reversal instruction.
    uint8_t reversed[16];
    const uint8_t* raw = reinterpret_cast<const uint8_t*>(v.bytes());
    for (size_t i = 0; i < 16; i++) {
      reversed[i] = raw[15 - i];
    }
    return m_buffer.allocEntry(slots, 4, (uint8_t*)inst, reversed);
#else
  ...
  void movePtr(ImmGCPtr imm, Register dest) {
    // Pool-load GC pointers where possible: 1-2 instructions and no inline
    // data in the execution stream, versus the 8-slot load64 stanza. The GC
    // reads and updates the pointer through the data relocation recorded
    // here; Extract/UpdateLoad64Value recognize the patched pool-load shape
    // and dereference into the pool entry.
    if (HasPOWER9() && !m_buffer.poolsInhibited() && !oom()) {
      BufferOffset bo =
          loadFromPoolGPR64(dest, (uint64_t)uintptr_t(imm.value));
  ...
  FaultingCodeRange load8SignExtend(const Address& address, Register dest) {
    ...
    if (is_intN(address.offset, 16)) {
      ...
      movePtr(ImmWord(address.offset), scratch);
      fco =
          FaultingCodeRange(as_lbzx(dest, address.base, scratch).getOffset());
    }
    as_extsb(dest, dest);
    return fco;
  }
  FaultingCodeRange load8SignExtend(const BaseIndex& src, Register dest) {
    ...
    FaultingCodeRange fco;
    if (is_intN(src.offset, 16)) {
      fco = FaultingCodeRange(as_lbz(dest, scratch, src.offset).getOffset());
    } else {
      MOZ_ASSERT(scratch != dest);
      movePtr(ImmWord(src.offset), dest);
      fco = FaultingCodeRange(as_lbzx(dest, scratch, dest).getOffset());
    }
    as_extsb(dest, dest);
    return fco;
  }
  FaultingCodeRange load8ZeroExtend(const Address& address, Register dest) {
    ...
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_lbzx(dest, address.base, scratch).getOffset());
  }
  FaultingCodeRange load8ZeroExtend(const BaseIndex& src, Register dest) {
    ...
    computeScaledAddress(src, scratch);
    if (is_intN(src.offset, 16)) {
      return FaultingCodeRange(as_lbz(dest, scratch, src.offset).getOffset());
    }
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(src.offset), dest);
    return FaultingCodeRange(as_lbzx(dest, scratch, dest).getOffset());
  }
  FaultingCodeRange load16SignExtend(const Address& address, Register dest) {
    ...
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_lhax(dest, address.base, scratch).getOffset());
  }
  FaultingCodeRange load16SignExtend(const BaseIndex& src, Register dest) {
    ...
    computeScaledAddress(src, scratch);
    if (is_intN(src.offset, 16)) {
      return FaultingCodeRange(as_lha(dest, scratch, src.offset).getOffset());
    }
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(src.offset), dest);
    return FaultingCodeRange(as_lhax(dest, scratch, dest).getOffset());
  }
  template <typename S>
  ...
  FaultingCodeRange load16ZeroExtend(const Address& address, Register dest) {
    ...
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_lhzx(dest, address.base, scratch).getOffset());
  }
  FaultingCodeRange load16ZeroExtend(const BaseIndex& src, Register dest) {
    ...
    computeScaledAddress(src, scratch);
    if (is_intN(src.offset, 16)) {
      return FaultingCodeRange(as_lhz(dest, scratch, src.offset).getOffset());
    }
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(src.offset), dest);
    return FaultingCodeRange(as_lhzx(dest, scratch, dest).getOffset());
  }
  template <typename S>
  ...
  FaultingCodeRange load32(const Address& address, Register dest) {
    ...
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_lwax(dest, address.base, scratch).getOffset());
  }
  FaultingCodeRange load32(const BaseIndex& address, Register dest) {
    ...
    if (is_intN(address.offset, 16)) {
      FaultingCodeRange fco(as_lwz(dest, scratch, address.offset).getOffset());
      as_extsw(dest, dest);
      return fco;
    }
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(address.offset), dest);
    return FaultingCodeRange(as_lwax(dest, scratch, dest).getOffset());
  }
  ...
  FaultingCodeRange loadPtr(const Address& address, Register dest) {
    ...
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_ldx(dest, address.base, scratch).getOffset());
  }
  FaultingCodeRange loadPtr(const BaseIndex& src, Register dest) {
    ...
    if (is_intN(src.offset, 16) && !(src.offset & 0x3)) {
      return FaultingCodeRange(as_ld(dest, scratch, src.offset).getOffset());
    }
    MOZ_ASSERT(scratch != dest);
    movePtr(ImmWord(src.offset), dest);
    return FaultingCodeRange(as_ldx(dest, scratch, dest).getOffset());
  }
  ...
  FaultingCodeRange loadDouble(const Address& addr, FloatRegister dest) {
    ...
    return FaultingCodeRange(as_lfdx(dest, addr.base, scratch).getOffset());
  }
  FaultingCodeRange loadDouble(const BaseIndex& src, FloatRegister dest) {
    ...
    computeScaledAddress(src, scratch);
    if (is_intN(src.offset, 16)) {
      return FaultingCodeRange(as_lfd(dest, scratch, src.offset).getOffset());
    }
    Register scratch2 = temps.Acquire();
    movePtr(ImmWord(src.offset), scratch2);
    return FaultingCodeRange(as_lfdx(dest, scratch, scratch2).getOffset());
  }
  FaultingCodeRange loadFloat32(const Address& addr, FloatRegister dest) {
    ...
    Register scratch = temps.Acquire();
    movePtr(ImmWord(addr.offset), scratch);
    return FaultingCodeRange(as_lfsx(dest, addr.base, scratch).getOffset());
  }
  FaultingCodeRange loadFloat32(const BaseIndex& src, FloatRegister dest) {
    ...
    computeScaledAddress(src, scratch);
    if (is_intN(src.offset, 16)) {
      return FaultingCodeRange(as_lfs(dest, scratch, src.offset).getOffset());
    }
    Register scratch2 = temps.Acquire();
    movePtr(ImmWord(src.offset), scratch2);
    return FaultingCodeRange(as_lfsx(dest, scratch, scratch2).getOffset());
  }
  ...
  FaultingCodeRange store8(Register src, const Address& address) {
    ...
    Register scratch = temps.Acquire();
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_stbx(src, address.base, scratch).getOffset());
  }
  FaultingCodeRange store8(Register src, const BaseIndex& address) {
    ...
    return FaultingCodeRange(as_stbx(src, scratch, scratch2).getOffset());
  }
  ...
  FaultingCodeRange store16(Register src, const Address& address) {
    ...
    Register scratch = temps.Acquire();
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_sthx(src, address.base, scratch).getOffset());
  }
  FaultingCodeRange store16(Register src, const BaseIndex& address) {
    ...
    Register scratch2 = temps.Acquire();
    movePtr(ImmWord(address.offset), scratch2);
    return FaultingCodeRange(as_sthx(src, scratch, scratch2).getOffset());
  }
  ...
  FaultingCodeRange store32(Register src, const Address& address) {
    ...
    Register scratch = temps.Acquire();
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_stwx(src, address.base, scratch).getOffset());
  }
  FaultingCodeRange store32(Register src, const BaseIndex& address) {
    ...
    Register scratch2 = temps.Acquire();
    movePtr(ImmWord(address.offset), scratch2);
    return FaultingCodeRange(as_stwx(src, scratch, scratch2).getOffset());
  }
  ...
  FaultingCodeRange storePtr(Register src, const Address& address) {
    ...
    Register scratch = temps.Acquire();
    movePtr(ImmWord(address.offset), scratch);
    return FaultingCodeRange(as_stdx(src, address.base, scratch).getOffset());
  }
  FaultingCodeRange storePtr(Register src, const BaseIndex& address) {
    ...
    Register scratch2 = temps.Acquire();
    movePtr(ImmWord(address.offset), scratch2);
    return FaultingCodeRange(as_stdx(src, scratch, scratch2).getOffset());
  }
