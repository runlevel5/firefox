...
enum PPCOpcodes {
  ...
  // Base (ISA 2.06) forms: NaN operands are suppressed in favor of the
  // numeric operand (minNum/maxNum style), so these are only ECMA-correct
  // once NaN inputs have been excluded.
  PPC_xsmaxdp = 0xF0000400,
  PPC_xsmindp = 0xF0000440,
...
class Assembler : public AssemblerShared {
 public:
  ...
  enum PoolLoadType {
    PoolLoadGPR64 = 0,    // ld rD, offset(rBase)
 ...
 public:
  ...
  size_t size() const;
  // Size of the committed buffer, excluding any constant pool currently
  // under construction; the readable extent for trap-site verification.
  size_t readableSize() const { return m_buffer.sizeExcludingCurrentPool(); }
 ...
 public:
  ...
  BufferOffset as_andc(Register rd, Register rs, Register rb);
  ...
  BufferOffset as_xsminjdp(FloatRegister xt, FloatRegister xa,
                           FloatRegister xb);
  BufferOffset as_xsmaxdp(FloatRegister xt, FloatRegister xa,
                          FloatRegister xb);
  BufferOffset as_xsmindp(FloatRegister xt, FloatRegister xa,
                          FloatRegister xb);
  ...
  // VSX SIMD load/store (DQ-form, ISA 3.0). The displacement must be a
  // 16-byte multiple in [-32768, 32752]; RA=0 means base 0, so r0 is
  // disallowed as a base.
  BufferOffset as_lxv(FloatRegister xt, Register ra, int32_t offset);
  BufferOffset as_stxv(FloatRegister xs, Register ra, int32_t offset);
  ...
  BufferOffset as_pstfs(FloatRegister frs, Register ra, int64_t imm34,
                        bool R);
  // 8RR:D splat-immediate forms (no R bit; prefix carries imm bits 31-16).
  // xxspltiw writes imm32 into all four word elements; xxsplti32dx writes
  // imm32 into word elements IX and IX+2 only, preserving the others.
  BufferOffset as_xxspltiw(FloatRegister xt, uint32_t imm);
  BufferOffset as_xxsplti32dx(FloatRegister xt, bool ix, uint32_t imm);

 private:
  // Emit a nop before a prefixed instruction iff the prefix would otherwise
  // start at offset 12 (mod 16), which would split the prefix and suffix
  // across a 16-byte boundary (a POWER10 alignment requirement).
  void ensurePrefixedAlignment();

  // Emit an already-encoded prefix+suffix word pair as a single, unsplittable
  // prefixed instruction: reserve the pool, align, write both words, and
  // return the offset of the prefix.
  BufferOffset emitPrefixedInst(uint32_t prefix, uint32_t suffix);
